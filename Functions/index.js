...
exports.sendWelcomeEmail = functions.database.ref('/basket/{uid}').onUpdate((event) => {
  // [END onCreateTrigger]
  // [START eventAttributes]
  const snapshot = event.data; // The Firebase user.
  ...
  mailOptions.subject = completed ? 'طلب شراء' : 'Cancel';
  mailOptions.text = completed ? 'شكرا لاهتمامك سيتواصل معك أحد موظفينا قريبا' : 'Cart is empty';

  // [END eventAttributes]

  return sendWelcomeEmail(mailOptions);
});
